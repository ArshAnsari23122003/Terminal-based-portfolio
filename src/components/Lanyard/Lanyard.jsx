...
function InteractiveCard() {
  ...
  // Rope/Spherical joints holding the card to the fixed anchor
  ...
  useFrame((state) => {
    // Parallax: camera drifts slightly with the pointer
    ...
    if (fixed.current && card.current && band.current) {
      ...
      // Aerodynamic stabilization so the card settles instead of spinning forever
  ...
  // Click the card to give it a spin
  const handleFlip = () => {
    if (card.current && !dragged) {
      card.current.setAngvel({ x: 0, y: 40, z: 0 }, true);
    }
  };
  ...
  return (
    <>
        ...
        <group scale={0.65}>
          ...
          <group position={[0, 0, 0.08]}>
            <mesh position={[0, 1.5, 0]}>
              ...
              <Text position={[0, 0, 0.01]} fontSize={0.12} color="black">
                ARSH ANSARI // 2026
              </Text>
            </mesh>

            <Text position={[-1, 0.8, 0]} fontSize={0.28} color="white" anchorX="left">
              ARSH ANSARI
            </Text>
            ...
            <group position={[-1, -0.6, 0]}>
              ...
              <Text position={[0, -0.25, 0]} fontSize={0.18} color="white" anchorX="left">LEETCODE: KNIGHT</Text>
              <Text position={[0, -0.5, 0]} fontSize={0.18} color="white" anchorX="left">CODECHEF: 4-STAR</Text>
            </group>

            <mesh position={[0.7, -1.3, 0]}>
              <circleGeometry args={[0.3, 32]} />
              <meshBasicMaterial color="#00ff00" opacity={0.6} transparent />
              <Text position={[0, 0, 0.01]} fontSize={0.1} color="black">VIP</Text>
            </mesh>
          </group>
          ...
          <mesh position={[0, 0, -0.08]} rotation={[0, Math.PI, 0]}>
            ...
            <Text position={[0, 0, 0.01]} fontSize={2.4} color="#00ff00" fillOpacity={0.08}>A</Text>
      ...
      {/* Floating hero text behind the card */}
